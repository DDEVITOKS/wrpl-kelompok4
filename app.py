...
def detail(id):
    cur = mysql.connection.cursor()
    cur.execute("SELECT * FROM products JOIN users ON products.user_id = users.id WHERE id_product=%s", (id,))
    data = cur.fetchall()
    cur.close()
...
def register():
    if request.method == 'GET':
        ...
        cur = mysql.connection.cursor()
        cur.execute("INSERT INTO users (name, email, password) VALUES (%s,%s,%s)",(nama,email,hash_password,))
...
def update():
    if 'loggedin' in session:
        if request.method == 'GET':
            ...
            cursor.execute("UPDATE users SET name =% s, email =% s, password =% s  WHERE id=%s",(username,email,hash_password,(session['id'], ),))
            mysql.connection.commit()
